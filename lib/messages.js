...
var parseMessages = function(network, dataBuffer) {
  $.checkArgument(network);
  $.checkArgument(dataBuffer);
  var messages = [];
  function parse() {
    var message = parseMessage(network, dataBuffer);
    if(message) {
...
function Ping(nonce) {
   ...
   * @type {number}
   */
  this.nonce = nonce || BufferUtil.EMPTY_BUFFER;
}
...
function Pong(nonce) {
  this.command = 'pong';
  /**
   * @desc A random number matching one sent in a previous `ping` message
   * @type {number}
   */
  this.nonce = nonce || BufferUtil.EMPTY_BUFFER;
}
